...
async function makeReimbursment() {

    let type_id = 0;

    if (document.getElementById("input").value == "Lodging") {
        type_id = 1;
    }
    if (document.getElementById("input").value == "Travel") {
        type_id = 2;
    }
    if (document.getElementById("input").value == "Food") {
        type_id = 3;
    }
    if (document.getElementById("input").value == "Other") {
        type_id = 4;
    }
    ...
    try {
        await fetch(`http://localhost:8000/Reimbursements/`, {
            method: 'POST',
            body: JSON.stringify({
